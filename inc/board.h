...
public:
	Board();
	Board(const uint32_t rows, const uint32_t cols); // FIXME: repeated code in reset
	...
	void reset(const uint32_t rows, const uint32_t cols); // FIXME: repeated code in constructor
	...
	// Operators
	// Board& operator=(const Board &other); // TODO: operator =
	friend void swap(Board &first, Board &second); // TODO: implement it!
